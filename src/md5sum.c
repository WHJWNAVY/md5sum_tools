 ...
 * md_context structure, pass it to md5_init, call md5_update as
...
static void byte_reverse(uint8_t *buf, uint32_t longs) {
    uint32_t t;
    do {
        t = (uint32_t)((uint8_t)buf[3] << 8 | buf[2]) << 16 | ((uint8_t)buf[1] << 8 | buf[0]);
        *(uint32_t *)buf = t;
        buf += 4;
    } while (--longs);
}
 ...
 */
static void md5_init(struct md_context *ctx) {
 ...
 * of bytes.
 */
static void md5_update(struct md_context *ctx, uint8_t const *buf, unsigned len) {
    register uint32_t t;
 ...
 */
static void md5_final(uint8_t digest[16], struct md_context *ctx) {
    uint32_t count;
    uint8_t *p;
...
void md5sum_str(uint8_t *input, int len, char *outstr) {
    ...
    md5sum(input, len, output);
    for (idx = 0; idx < sizeof(output); idx++) {
        sprintf(pstr, "%02X", output[idx]);
        pstr += 2;
    }
    *pstr = '\0';
}
